...
int v, e, x, y, z, pt, a, b, d, temp, hw, spd, mnt, wt, weight, jam, menit, vhc_now, prev_vhc;
int dist[105], travel_time[105];	//menyimpan jarak terpendek, waktu tempuh tercepat dari vertex asal ke masing-masing vertex lainnya
prv prev[105];						//menyimpan informasi indeks vertex yang dilalui sebelumnya
...
void shortest_route(int now){
	if(prev[now].vertex == now)
		cout<<now;
	else{
		shortest_route(prev[now].vertex);
		cout<<" - "<<now;
	}
	return;
}

void fastest_route(int now){
	if(prev[now].vertex != now){
		fastest_route(prev[now].vertex);
		cout<<"\n\tdari titik "<<prev[now].vertex<<" menunggu "<<prev[now].waiting_time / 60<<" menit "<<prev[now].waiting_time % 60<<" detik, kemudian naik "<<pub_transp[prev[now].vehicle].name<<" ke titik "<<now;
	}
	return;
}
...
void find_shortest_path(){
	...
	if(dist[b] == maks)	//jika jarak pada vertex tujuan = infinite (berarti tidak ditemukan rute)
		...
		cout<<" - Jarak terpendek = "<<dist[b]<<" m"<<endl<<" - Rute : ";
		shortest_route(b);
		cout<<endl;
	}
}
...
void find_fastest_path(){
	...
	pq.push(make_pair(menit*60,a));	//insert vertex awal
	...
	while(!pq.empty()){
		...
		for(int i=0; i<adj[x].size(); i++){
			y = adj[x][i].first;			//index vertex tetangga
			d = adj[x][i].second;			//waktu tempuh dari vertex x menuju vertex tetangga ke-i
			...
			for(int j=0; j<option[x][y].size(); j++){
				...
				else wt = hw - (mnt % hw);		//waktu tunggu (detik)
...
//					cout<<x<<' '<<y<<' '<<vhc_now<<' '<<travel_time[y]-travel_time[a]<<" ("<<weight<<' '<<d/spd<<' '<<wt<<')'<<endl;
	...
	if(travel_time[b] == maks)	//jika jarak pada vertex tujuan = infinite (berarti tidak ditemukan rute)
		...
		cout<<" - Waktu tempuh tercepat = "<<(travel_time[b] - travel_time[a]) / 60<<" menit"<<endl<<" - Rute : ";
		fastest_route(b);
		cout<<endl;
	} 	
}
...
int main(){
	...
	cout<<"(titik asal, titik tujuan, jarak)"<<endl;
	for(int i=0; i<e; i++){
		cin>>x>>y>>z;		//jalan menghubungkan titik x ke y dan jaraknya z
	...
	cout<<"(nama, kecepatan (km/h), headway (menit), jumlah titik yang dilalui, dan titik mana saja yang dilalui)"<<endl;
	for(int i=0; i<pt; i++){
		cin>>pub_transp[i].name>>pub_transp[i].speed>>pub_transp[i].headway>>z;
		
		//menerima inputan titik / tempat mana saja yang dilewati oleh rute kendaraan ke-i
		//lalu menambahkan kendaraan ke-i kedalam array opsi kendaraan pada setiap ruas jalan yang dilaluinya
		cin>>temp;
		x = temp;
		for(int j=1; j<z; j++){
			cin>>y;
